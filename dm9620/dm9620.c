...
int dm9620_open(struct net_device *dev)
{
	struct usbnet *pusbnet = netdev_priv(dev);
	// container_of(dev, struct usbnet, net);
	int ret,tmp;
	if ( (ret= dm_read_reg(pusbnet, 0x00, &tmp)) >=0)
		printk("%s(): %d %02x\n", __FUNCTION__, __LINE__, tmp);
	else
		printk("Fail%s(): %d %02x\n", __FUNCTION__, __LINE__, tmp);

	tmp &= ~(1 << 10);
	dm_write_reg(pusbnet, 0x00, &tmp);

	dm9620_open(dev);
}
int dm9620_stop(struct net_device *dev)
{
	struct usbnet *pusbnet = netdev_priv(dev);
	int ret,tmp;
	if ( (ret= dm_read_reg(pusbnet, 0x00, &tmp)) >=0)
...
static const struct net_device_ops vm_netdev_ops= { // new kernel 2.6.31  (20091217JJ)
	    ...
	    // .ndo_open               = dm9620_open,  
	    // .ndo_stop               = dm9620_stop,  
