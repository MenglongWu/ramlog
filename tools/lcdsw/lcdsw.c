...
ssize_t  lcd_write(struct file *filp, const char __user *buf, size_t size, loff_t *ppos);
ssize_t  lcd_read(struct file *filp, char __user *buf, size_t size, loff_t *ppos);
...
void display_off(unsigned long *ptr)
{
	u32 cfg;

	cfg = readl(ptr + S3C_VIDCON0);
	cfg &= ~S3C_VIDCON0_ENVID_ENABLE;
	writel(cfg, ptr + S3C_VIDCON0);

	cfg &= ~S3C_VIDCON0_ENVID_F_ENABLE;
	writel(cfg, ptr + S3C_VIDCON0);
}

int display_on(unsigned long *ptr)
{
	u32 cfg;
	...
	// dev_dbg(ptr, "global display is on\n");
